...
func (e *ExistenceConf) confIndustry(input *dpfm_api_input_reader.SDC) *dpfm_api_output_formatter.Industry {
	exconf := dpfm_api_output_formatter.Industry{
		ExistenceConf: false,
	}
	if input.Industry.Industry == nil {
		return &exconf
	}
	exconf = dpfm_api_output_formatter.Industry{
	...
	rows, err := e.db.Query(
		`SELECT Industry 
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_industry_industry_data 
		WHERE Industry = ?;`, exconf.Industry,
	)
	if err != nil {
		e.l.Error(err)
		return &exconf
